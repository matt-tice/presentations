...
Let $\mathcal{P}$ be a coloured properad in a symmetric monoidal category with all small coproducts $\mathcal{V}$, where the monodial product distributes over all \emph{finite} coproducts. For any small category $\mathcal{C}$, there exists a properad $\mathcal{P}^{\mathcal{C}}$ whose algebras are the functors
\begin{equation*}
...
* Explicit Form (infinite distributivity)
...
\[
A^{^{i}} \to X_{1} \to Y^{j} \to X_{2} \to Y^{k} \to \ldots \to B^{m}
\]
...
- BV-BRST formalism
- Aharonov-Bohm effect (Wilson loops are non-local)
...
- This is an easier model for performing computations
...
A /bialgebra/ is a unital associative $k$-algebra, $B$, with maps 

\medskip

- \fbox{Comultiplication: } $\Delta: B \to B \otimes B$

- \fbox{Counit: } $\varepsilon: B \to k$

\bigskip

satisfying the appropriate compatibility conditions between the algebraic and coalgebraic structure.

A /Laplace pairing/ on a bialgebra is a bilinear form $\gen{\cdot, \cdot}: B \otimes B \to k$ satisfying

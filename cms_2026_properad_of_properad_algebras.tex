...
\maketitle
\begin{frame}[label={sec:org99775e4}]{Main Results}
...
\end{proposition}
\end{frame}
\begin{frame}[label={sec:org017f4a1}]{Main Results (Properad Version)}
...
\end{proposition}
\end{frame}
\begin{frame}[label={sec:orgf3f4310},fragile]{Generators and Relations}
...
\]
\end{frame}
\begin{frame}[label={sec:orge46ea46}]{Explicit Form (infinitary distributivity)}
...
\]
\end{frame}
\begin{frame}[label={sec:orgc598fb0}]{Precursors}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org0d992e9},fragile]{Orthogonal Categories}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org7c24787}]{Causality}
...
whenever \(f \perp g\).
\end{frame}
\begin{frame}[label={sec:orgb032547}]{Introducing Homotopy}
\begin{itemize}
\item Homotopy allows us to capture gauge theories
\item Most common approaches: bar-cobar (ex. chain complexes), W-construction
\end{itemize}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orge818c53}]{Pointwise A\textsubscript{\(\infty\)}-algebras}
...
\end{frame}
\begin{frame}[label={sec:org4749f21}]{Pointwise A\textsubscript{\(\infty\)}-algebras}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org640b6b3},fragile]{Hopf-Laplace Algebras}
 A \emph{Hopf algebra} is a unital associative \$k\$-algebra, \(H\), with maps 
...
A \emph{Laplace pairing} on a Hopf algebra is a bilinear form \(\gen{\cdot, \cdot}: H \otimes H \to k\) satisfying
...
\end{frame}
\begin{frame}[label={sec:org1e7f6ec}]{Properad of Laplace Hopf AQFTs}
